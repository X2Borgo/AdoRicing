...
if wallust theme --skip-sequences -- "${choice}"; then
  have_notify && notify-send -a ThemeChanger \
    -h string:x-dunst-stack-tag:themechanger \
    "Global theme changed" "Selected: ${choice}"

  # Wait until shell-owned template targets exist, are newer than start_ts, and are stable (size/mtime stops changing).
  # Rofi, Kitty, Ghostty, and Waybar use static themes in this setup and are intentionally not Wallust-driven.
