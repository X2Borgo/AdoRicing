...
- `SYMLINK_MIGRATION_PLAN.md` documents the in-progress move from copy-based install to symlink-based install (repo as live source of truth). Follow it when touching `install.sh` — e.g. Zed config is being split into committed `settings.shared.json` vs ignored `settings.local.json`, and copied-then-chmod patterns must not be applied to symlinked trees.
