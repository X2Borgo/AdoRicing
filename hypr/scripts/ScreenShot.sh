...
shotcliparea() {
	...
	if [[ -s "$tmpfile" ]]; then
		wl-copy --type image/png <"$tmpfile"
		"${sDIR}/Sounds.sh" --screenshot
		notify-send -t 3000 -i "${iDIR}/picture.png" "Screenshot" "Copied to clipboard"
	fi
...
if [[ "$1" == "--now" ]]; then
	...
	shotactive
elif [[ "$1" == "--clip-area" ]]; then
	shotcliparea
elif [[ "$1" == "--swappy" ]]; then
	shotswappy
else
	echo -e "Available Options : --now --in5 --in10 --win --area --active --clip-area --swappy"
fi
