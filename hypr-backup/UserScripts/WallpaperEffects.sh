...
no-effects() {
    ...
    wait $!
    ADO_KEEP_FIXED_PALETTE=1 "$SCRIPTSDIR/WallustSwww.sh" "$wallpaper_current" &&
    wait $!
    # Refresh shell components after wallpaper update
	sleep 2
...
main() {
    ...
    if [[ -n "$choice" ]]; then
        if [[ "$choice" == "No Effects" ]]; then
            ...
            ADO_KEEP_FIXED_PALETTE=1 "$SCRIPTSDIR/WallustSwww.sh" "$wallpaper_output" &
            sleep 1
            # Refresh shell components after wallpaper update
            "${SCRIPTSDIR}/Refresh.sh"
